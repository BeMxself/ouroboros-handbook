...
这页主要作为后端扩展的详细实现入口；当你已经确认需求需要通过代码扩展落地，再从这里进入具体扩展点。
...
2. 扩展实现本身，例如 `ELContextWrapper`、`ScriptContextWrapper`、`NodeBuilder`、`FieldTypeDefinitionProvider`。
3. 让平台发现它的注册信息，常见是 `META-INF/services/*`、`META-INF/ouroboros/*`、`META-INF/spring.factories`。
